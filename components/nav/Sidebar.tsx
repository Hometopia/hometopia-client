...
export default function Sidebar({ data }: { data: NavigationItemType[] }) {
  ...
  const handleActive = (item: NavigationItemType) => {
    setState((prev) =>
      prev.map((i) => {
        ...
        } else i.state = SidebarItemState.default;
        return i;
      }),
    );
  };
  ...
  const bgStateColor = ["bg-white", "bg-background-50", "bg-primary-400"];
  ...
  return (
    <VStack
      style={collapse ? { width: "auto" } : { width: 200 }}
      className={`h-full justify-between border-r border-outline-300 px-4 py-6`}
    >
      <VStack className="gap-6">
        <VStack className="items-end gap-4">
          <div
            className="flex justify-center"
            style={collapse ? { width: "100%" } : { width: "fit-content" }}
            onClick={() => setCollapse((prev) => !prev)}
          >
            {collapse ? (
              <PanelLeftOpenIcon className="text-typography-600 hover:cursor-pointer" />
            ) : (
          ...
          >
            {collapse ? (
              <Image source={`${IMAGE_URL}/logo-only.svg`} className="h-[30px] w-[30px]" />
            ) : (
              <Image source={`${IMAGE_URL}/logo-full.svg`} className="h-[30px] w-[120px]" />
            )}
          </div>
        </VStack>
        <VStack className="gap-2">
          {data.map((i, index) => (
            <div
              key={i.slug}
              className={`flex items-center gap-3 rounded-md px-3 py-2 ${bgStateColor.at(state.at(index)?.state as number)} hover:cursor-pointer`}
            ...
            >
              <i.icon
                className={` ${stateColor.at(state.at(index)?.state as number)}`}
                strokeWidth={2}
                size={20}
              />
              {collapse ? (
                ...
                <Text
                  className={`text-sm font-normal capitalize ${stateColor.at(state.at(index)?.state as number)}`}
                >
                  {i.label}
                </Text>
              )}
