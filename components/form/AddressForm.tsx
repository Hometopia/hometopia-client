...
const AddressSectionSelect = ({ items }: { items: { label: string, value: string }[] }) => (
  <Select >
  ...
  return (
    <VStack className="flex flex-col w-96 justify-center gap-12">
      <VStack className="gap-4">
        <Text className="text-center text-3xl font-bold text-typography-900">Địa chỉ</Text>
        <Text className="text-center text-sm font-normal text-typography-500">Cho chúng tôi biết địa chỉ của bạn.
          Điều này sẽ giúp chúng tôi có thể tìm kiếm các dịch vụ gần nơi bạn ở.</Text>
      </VStack>

      <FormControl className="gap-8 z-10">
        <VStack className="gap-4 z-10">
          <VStack className="z-auto">
            <Text className="text-sm text-typography-500">Tỉnh / Thành phố</Text>
            {platform === 'web' ?
              <CustomSelect placeholder='Tỉnh / Thành phố' value={city} onChange={setCity} /> :
